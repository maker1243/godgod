...
const BLESSING_COMBOS = [
  ...
  { id:'combo_holyshield',
    name:'HOLY SHIELD',
    color:'#ffffff',
    need: ['bl_holyward', 'bl_lastward', 'bl_immortal'],
    desc:'HOLY + LAST WARD + IMMORTAL → HP 5 이하 자동 회복',
    apply:(p)=>{ p._synHolyShield = true; },
  },
  { id:'combo_deathdance',
    name:'DEATH DANCE',
    color:'#ff2d80',
    need: ['bl_deathmark', 'bl_soulharvest', 'bl_hex'],
    desc:'DEATH MARK + SOUL HARVEST + HEX → 킬 시 다음 히트 크리티컬 확정',
    apply:(p)=>{ p._synDeathDance = true; },
  },
  { id:'combo_stormlord',
    name:'STORM LORD',
    color:'#8bd8ff',
    need: ['bl_thunder', 'bl_chainlt', 'bl_echo'],
    desc:'THUNDER + CHAIN + ECHO → 모든 히트가 3인접 몹에 30% 데미지',
    apply:(p)=>{ p._synStormLord = true; },
  },
];
